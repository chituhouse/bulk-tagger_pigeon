...
## ✨ 更新亮点（2025-07-04）
1. **✅ 完全支持 OpenRouter `minimax/minimax-01` 视觉模型**，开箱即用，无需等待官方 API。
2. **🖼️ 图片预览面板**：GUI 右侧实时显示所选图片，所见即所得。
3. **📄 自动 TXT 导出**：每张图片处理完成后立即生成同名 `.txt` 文件。
4. **⚡ 逐张顺序处理**：严格符合一次一张请求等待返回再处理下一张的需求。
5. **🔒 稳定错误处理**：完善 429 限流、500 服务器错误的自动重试与提示。

> 当前功能完成度已达到 **80%+**，核心流程全部跑通，可在生产环境小规模使用。
